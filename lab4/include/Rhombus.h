...
public:
    ...
    {
        return this->pt_dist(this->points[0], this->points[2]) * this->pt_dist(this->points[1], this->points[3]) / 2;
    }

    template <Scalar D>
    friend std::ostream &operator<<(std::ostream &os, Rhombus<D> &f) noexcept;

    template <Scalar D>
    friend std::istream &operator>>(std::istream &is, Rhombus<D> &f) noexcept;
};

template <Scalar T>
std::ostream &operator<<(std::ostream &os, Rhombus<T> &f) noexcept
{
...
template <Scalar T>
std::istream &operator>>(std::istream &is, Rhombus<T> &f) noexcept
{
